...
### 2026-06-10 (ночь) — Менеджерская панель (план утверждён Hiki)

Аудит опыта: клиентский MVP готов end-to-end, менеджерский = только API (вести бизнес
без curl нельзя — блокер первого заказа). План ратифицирован → построена панель.

**Сделано (`68e9366`, задеплоено на прод Vercel):**
- Третья Vite-entry: `manager.html` + `src/manager/` (паттерн WebApp, общая дизайн-система).
- Вход email+пароль → JWT в sessionStorage (XSS-компромисс — техдолг), 401 → авторазлогин.
- Экраны: лиды (фильтр + смена статуса), клиенты (+ «завести груз»), грузы (список с
  lane/status-бейджами, форма создания с decimal-полями, детали: таймлайн, смена статуса,
  чат с клиентом, платежи — запись и смена статуса).
- Попутно: lane-бейдж в клиентском WebApp (карточка + детали).
- E2E локально в браузере: вход → лид contacted → груз (white, 1800.50 USD) → статус
  in_transit → платёж 900.25 card_sbp → confirmed → сообщение в чат. Всё прошло.
- Прод проверен: `iracus-logistics.vercel.app/manager.html` отдаёт свежий бандл с
  запечённым Railway-URL.

**Хвосты для Hiki:**
- Завести себе менеджера в прод-БД Railway (пароль — твой выбор, не свети в чате):
  `cd backend && DATABASE_URL='<public url БД>' go run ./cmd/createmanager -email=... -name=... -password=...`
  и проверить вход на `/manager.html`.
- Техдолг #23 (новый): manager-JWT в sessionStorage — при реальных деньгах перейти на
  httpOnly-cookie (требует бэкенд-изменений).

### Закрытие сессии 2026-06-10 — итог дня и план на завтра

**Сделано за день (всё запушено, прод обновлён):** README/PROGRESS актуализированы;
качественный проход по лендингу (мотив коридора, степпер, редакционные услуги) + онбординг
пустого WebApp; staging подтверждён рабочим end-to-end; **бэкенд-роадмап целиком** —
платежи (v10), lane (v11), доверенные прокси (#20), outbox (v12, #11/#15), webhook-режим,
фиксы по ревью (таймаут HTTP-клиента бота — head-of-line outbox'а, 204 на битый апдейт,
WithoutCancel в пометках); Railway мигрирован до v12; **менеджерская панель** (третья
entry: лиды/клиенты/грузы/статусы/чат/платежи), E2E локально, прод-деплой; lane-бейдж в
клиентском WebApp.

**Инциденты дня (уроки записаны в память):**
- Ревью-воркфлоу породил ~20 агентов на дорогой модели — впредь флоты только с ОК Hiki,
  объёмная работа на дешёвых моделях (память cost-discipline).
- Локальный `go run ./cmd/api` поднял бота с боевым токеном из backend/.env → дрался за
  long polling с Railway (409) и слал реальные уведомления из локальной БД. Симптом у Hiki:
  «уведомление пришло, а в WebApp груза нет» — груз был в локальной базе. Локальный API
  убит. **Правило: перед локальным запуском API убедиться, что TELEGRAM_BOT_TOKEN пуст.**

**ПЛАН НА ЗАВТРА (по приоритету):**
1. **Claude — инвойс-цикл** (начат, откачен до чистого состояния): уведомление клиенту
   при создании платежа («выставлен счёт», pending) и подтверждении («платёж получен»)
   через outbox; клиентская ручка `GET /api/app/shipments/:id/payments` (проверка
   принадлежности как DetailForClient); блок «Платежи» в WebApp-деталях (graceful при
   404 до деплоя бэка). PaymentService получит clients/notifier/bg — конструктор и тесты
   обновить.
2. **Hiki — прод-обвязка:** создать менеджера в Railway-БД (`createmanager` на публичный
   URL), войти в `iracus-logistics.vercel.app/manager.html`, завести реальный груз для LL
   и проверить его в WebApp; убрать боевой TELEGRAM_BOT_TOKEN из backend/.env; дополнить
   .env.example (TRUSTED_PROXIES, TELEGRAM_WEBHOOK_URL/SECRET).
3. **Hiki — ревью** бэкенд-итерации по чеклисту `docs/learning-phase-b-infra.md`.
4. Опционально: webhook на Railway (#22); периодическое обновление списка в WebApp;
   ротация засвеченных секретов; удалить орфан `icaris-logistics` на Vercel.
5. Горизонт: контентная глубина лендинга (фото склада Гуанчжоу); юр. дорожка (ККТ,
   152-ФЗ) — вне кода.

### 2026-06-11 — Инфра-уборка + инвойс-цикл

**Уборка (вместе с Hiki):**
- Vercel: орфан `icaris-logistics` уже удалён, остался канонический `iracus-logistics`
  (все деплои READY, прод отдаёт свежий main).
- Railway: обнаружен дубль-проект `brilliant-gratitude` (icaris-db Online ел trial-кредит,
  icaris-api с failed build). Проверено по домену: основной — **`helpful-art`**
  (`icaris-api-production-7695.up.railway.app`, health 200). Дубль удалён Hiki.
- Webhook (#22): разобрали, что это и зачем; решено включить в конце очереди задач.

**Инвойс-цикл (`67ada47` бэкенд, `42313c7` фронтенд):**
- Бот: `NotifyPaymentCreated`/`NotifyPaymentConfirmed` + формат-функции с русскими
  подписями каналов; уведомления идут через deliver → outbox (ретраи бесплатно).
- `PaymentService` + clients/notifier/bg: pending при создании → «выставлен счёт»;
  сразу confirmed или переход pending→confirmed → «платёж получен»; ре-confirm и
  refund дублей не шлют. Фон — паттерн notifyClient.
- Клиентская ручка `GET /api/app/shipments/:id/payments` (принадлежность как
  DetailForClient: чужой груз → 404).
- WebApp: блок «Платежи» в деталях (сумма/канал/статус-бейдж/дата/комментарий);
  при пустом списке или любой ошибке (вкл. 404 до деплоя бэка) секция скрыта.
- Тесты: +8 в payment_test (уведомления через дренаж Background, ListForClient);
  build/vet/gofmt/test зелёные; репо-тесты прогнаны на живой локальной БД (v12).
- E2E локально (API с пустым TELEGRAM_BOT_TOKEN — урок инцидента 06-10, изолированный
  env): логин менеджера → платёж 1500.75 card_sbp → confirm; клиентский JWT → свой
  груз отдаёт платежи, чужой → 404; UI проверен Playwright (блок рендерится).

**Заметка:** клиентская ручка платежей отдаёт `created_by` (UUID менеджера) — то же
поведение, что у деталей груза (manager_id); отдельные DTO по аудиториям — кандидат
в техдолг, не блокер.

**Следующий шаг:** пуш + ревью Hiki; после деплоя бэка на Railway блок в WebApp оживёт
сам. Дальше по плану: прод-обвязка Hiki (менеджер в прод-БД, чистка .env, .env.example),
ротация секретов, webhook (#22) — в конце.

### 2026-06-11 (продолжение) — Аудит v2 (экономный)

Воркфлоу 12 агентов на sonnet (~570k токенов против 3.5M вчера): 4 аудитора
(security-backend, security-frontend+repo, dead-code, docs) + верификация major-находок.

**Подтверждено (7 major):** нет лимита тела на публичных POST; нет пагинации в List;
JWT без отзыва (TTL 24h); TrustedProxies слишком широкие; пароль `secret` в README;
фейковые ИНН/ОГРН на лендинге (TODO hiki); PDF-блобы остались в git-истории.
Отклонено: dev `?token` (вырезается в prod-сборке). Всё оформлено в tech-debt #24–28.

**Сделано по горячим следам:**
- PDF внутренних аудитов убраны из трекинга + `*.pdf` в .gitignore (`fc4aad8`);
  блобы в истории остались — filter-repo за Hiki.
- Чистка (`977932d`): App.css (364 строки мёртвого прототипа), STATUS_FLOW; README
  актуализирован (v12, три entry, ручка платежей, webhook/outbox, пароль-плейсхолдер).
- `go mod tidy` чист; `.mcp.json` без секретов (ключ через env) — оставлен.
- Ложная находка отсеяна: `.env.example` существуют (агент смотрел `ls` без `-a`).

**Открыто (решение Hiki):** делать ли security-фиксы #24–27 сейчас или отдельной фазой;
filter-repo по PDF; скрыть реквизиты в Footer.

**Заметка окружения:** npm/vite/tsc стали зависать на старте (без вывода); лечится
kill + повтор, tsc — чисткой tsbuildinfo. Возможно, давление памяти от Docker Desktop.

### 2026-06-11 (вечер) — Security-фиксы #24-27 + чистка истории git

- **#24 закрыт:** `middleware.MaxBodyBytes` 64 КиБ на `/api` (проверено: 100 КиБ → 400).
- **#25 закрыт:** пагинация `?limit&offset` (дефолт 100/потолок 200, `service.Page`)
  для лидов/клиентов/грузов; дочерние списки — `Limit(500)` в репо; бот /status — 50.
  Проверено вживую на локальной БД.
- **#26 минимум:** дефолт `JWT_TTL` 24h → 4h; полный revocation остаётся в техдолге.
- **#27 частично:** warning на старте прода без `TRUSTED_PROXIES`; значение на Railway — Hiki.
- **История git переписана** (`git filter-repo`): PDF-блобы внутренних аудитов удалены,
  `.graphifyignore` убран из трекинга; force push в GitHub. Хеши коммитов после `2b7022a`
  изменились (старые ссылки в журнале выше — до перезаписи). Бэкап —
  `/tmp/icaris-backup-pre-filter.bundle`. Осиротевшие объекты на стороне GitHub живут
  до их GC; ускорение — только тикет в поддержку.
- Минорки аудита остаются пакетом в техдолге #28.

### 2026-06-11 (ночь) — UX-аудит + эксперты (инвестор, логист)

Воркфлоу: 4 UX-сценария (sonnet) + инвестор и логист на opus (~470k токенов).
Полный отчёт — `docs/audit-ux-expert-2026-06-11.md`. Сжатая суть:

- **UX-блокеры:** уведомление о счёте без реквизитов (клиенту некуда платить —
  разрыв инвойс-цикла); произвольный текст в боте молча игнорируется; фейковые
  ИНН/ОГРН в футере (известно). Major-россыпь: меню бота не объясняет как открыть
  Mini App, «грузов нет» без контекста после /start, панель без непрочитанных/
  фильтров/имени клиента/итога по платежам.
- **Инвестор (вердикт):** код зрелый, но инвест-кейса нет, пока: (а) нет договора
  с партнёром (non-circumvention) — задача №1, (б) юр. блокеры не сняты, (в) unit-
  экономика не измеряется (в Shipment нет себестоимости/комиссии партнёра).
- **Логист (вердикт):** модель — «курьер», не сборное карго. 4 critical: нет
  declared vs actual вес/объём (перевзвешивание сломает цену), нет мест/коробок,
  в выкупе деньги за товар и доставку смешаны в Payment, нет файлов (инвойсы/фото).
  Заболит в первый месяц реальной работы.

Кнопка «Ссылка для клиента» в лидах + реальные контакты @hikill8 — сделаны ранее
вечером (`4ae2fad`, `ac6a978`).
