...
### Закрытие 2026-06-11 — решения Hiki и план

**Решения Hiki (ратифицировано):**
- Юр. вопросы (ККТ, налоговый узел, договор с партнёром) — Hiki решает сам, начал сегодня.
- 152-ФЗ хостинг-локализация — осознанно отложена («каждый ТГ-бот на Vercel стоит»),
  на этом этапе не блокер. Вернуться при реальных объёмах ПДн.
- MVP объявлен завершённым. Следующий этап — расширенный план конечного продукта (v1.0),
  отправная точка — три аудита (код 06-09, security/UX/эксперты 06-11).
- Замечание Hiki: WebApp скуден — клиент видит заказ и ничего не может с ним сделать.
  Клиентские действия = ядро v1.0.

**ПЛАН НА ЗАВТРА (2026-06-12):**
1. **Claude — быстрые UX-фиксы по аудиту (полдня):**
   а) Реквизиты оплаты: клиент должен знать КУДА платить — поле реквизитов на платеже
      (или пресеты каналов), в уведомлении бота и в WebApp-карточке. Закрывает UX-блокер.
   б) Бот: ответ на произвольный текст (сейчас молчит) — меню + «вопрос по грузу — в
      приложении, срочное — @hikill8».
   в) Контекстные тексты: /start по deep-link («вы привязаны к заявке...»), пустые
      «Мои грузы» с объяснением следующего шага, инструкция как открыть Mini App.
   г) Панель: имя клиента на карточке груза, итог «оплачено/остаток» в платежах,
      фильтр грузов по статусу.
2. **Вместе — сессия планирования v1.0** (по аудитам, на ратификацию):
   - домен: declared vs actual вес/объём, места (cartons), purpose у Payment
     (товар/доставка/таможня), документы/файлы, exception-статусы, фазы карго;
   - WebApp-действия клиента: подтвердить получение, запрос выкупа, повторный заказ,
     документы, оплата с инструкцией;
   - операционка: непрочитанные, фильтры, себестоимость/маржа на грузе;
   - порезать на фазы, решить кто что пишет (value-based split).
3. **Hiki:** юр. дорожка (начата); ревью коммитов за 2026-06-11; `TRUSTED_PROXIES`
   на Railway; webhook #22 — в конце очереди.
