...
B2B-сервис экспедирования грузов **Китай → Россия**: публичный лендинг со сбором параметров и
...
- **Лендинг** — описание услуги, сбор параметров груза, форма заявки → `Lead` в БД.
...
1. Посетитель на лендинге готовит параметры груза и оставляет заявку (`Lead`) — без регистрации.
...
Обычный `npm run dev` оставляет публичную форму выключенной, пока нет действующей политики.
Для проверки полного пути заявки только на локальном компьютере используйте:
...
Этот режим подключает явно тестовое уведомление `privacy.local.html`. Оно не является
юридическим документом, предназначено только для вымышленных данных и не входит в production-сборку.
...
Команда работает только в `APP_ENV=development`, создаёт/обновляет demo-менеджера, клиента с
полным сценарием, нового клиента без грузов, груз, переписку и платёж, затем печатает локальные
ссылки и данные для входа. Повторный запуск не дублирует demo-сущности.
