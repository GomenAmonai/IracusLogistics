...
- На задеплоенном фронте (Vercel): задать `VITE_API_BASE` на публичный backend (домен фронта должен быть в `ALLOWED_ORIGINS` бэкенда; авторизация по Bearer — куки не используются).
