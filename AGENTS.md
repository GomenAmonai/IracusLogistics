...
Фаза 3 — Клиентский поток (сделано)
...
Фаза 4 — Грузы и статусы (сделано)

CRUD Shipment
Обновление статуса + комментарий
Уведомление клиента при смене статуса
История статусов (таблица shipment_status_events, миграция 000008)

Фаза 5 — WebApp (сделано)
...
Фаза 6 — Публичная часть (сделано)
...
Все фазы MVP (1–6) сделаны. Схема **v8**. Бэкенд: домен, миграции, репозитории, сервисы,
хендлеры; auth менеджера (JWT, role=manager) и клиента (Telegram initData → JWT role=client);
CRUD лидов и грузов; история статусов; чат; бот (long polling: /start, /status + уведомления
о статусе и сообщениях). Фронтенд: лендинг + Telegram Mini App (`webapp.html`, общая
дизайн-система). `go build/vet/test` и `npm run build` зелёные; потоки проверены end-to-end.
Дальше — прод-готовность (см. `docs/tech-debt.md`: webhook, outbox, CORS-whitelist,
обязательный JWT_SECRET, ReleaseMode) и ратификация решений из `docs/learning-phase3-5.md`.
